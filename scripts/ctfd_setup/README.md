...
| `deploy/docker-compose.workshop.yml` (+ optional `docker-compose.workshop.nvidia.yml`) | Brings up **docker-socket-proxy**, **CTFd**, and (by default) **one** shared **Ollama** container. **GPU** is requested only when the bootstrap/redeploy scripts merge **`docker-compose.workshop.nvidia.yml`** (when `nvidia-smi` works, or **`PWNZZAI_OLLAMA_GPU=1`**). CPU-only hosts (typical cloud VMs) omit that file so Docker does not error on CDI/GPU. The Ollama service uses Compose profile **`local-ollama`** (enabled by default via **`COMPOSE_PROFILES`**). |
...
- **GPU** is optional: merge **`deploy/docker-compose.workshop.nvidia.yml`** (scripts do this automatically when **`nvidia-smi`** lists a GPU, unless **`PWNZZAI_OLLAMA_GPU=0`**). Install the **NVIDIA Container Toolkit** on GPU hosts. On **CPU-only** machines, do not set **`PWNZZAI_OLLAMA_GPU=1`**; the default path runs Ollama on CPU.
